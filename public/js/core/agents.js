 ...
 * the arena wallet connection, and the on-chain push flow.
 */
...
const AgentArena = {
  ...
  initCards() {
    ...
    // Wire up arena wallet connect button
    const arenaWalletBtn = document.getElementById('btn-arena-connect-wallet');
    if (arenaWalletBtn) {
      arenaWalletBtn.addEventListener('click', async () => {
        try {
          const result = await Web3Client.connect();
          AgentArena._renderArenaWalletState();
          UI.log(`Wallet connected: ${Web3Client.shortAddress()} on ${Web3Client.networkName()}`);
          UI.toast('Wallet connected!', 'success');
          // Also update the auction tab wallet state
          if (typeof AuctionFlow !== 'undefined' && AuctionFlow._renderWalletState) {
  ...
  /** Render the arena wallet connection state */
  _renderArenaWalletState() {
    const stateEl = document.getElementById('arena-wallet-state');
    const btn = document.getElementById('btn-arena-connect-wallet');
    if (!stateEl) return;

    if (Web3Client.connected) {
      stateEl.innerHTML = `
        <span style="color:var(--terminal-green);">CONNECTED</span>
        <span class="web3-address" style="margin-left:0.5rem;">${Web3Client.shortAddress()}</span>
        <span style="color:#6b7280;margin-left:0.5rem;">${Web3Client.networkName()}</span>
      `;
      if (btn) {
      ...
      stateEl.innerHTML = '<span style="color:var(--terminal-red)">NOT CONNECTED</span>';
      if (btn) {
        ...
        btn.onclick = async () => {
          try {
            ...
            AgentArena._renderArenaWalletState();
            UI.log(`Wallet connected: ${Web3Client.shortAddress()}`);
            if (typeof AuctionFlow !== 'undefined' && AuctionFlow._renderWalletState) {
              AuctionFlow._renderWalletState();
            }
          } catch (err) {
    ...
    // Also update push button state
    if (typeof AuctionFlow !== 'undefined' && AuctionFlow._updatePushButton) {
      AuctionFlow._updatePushButton();
    }
  },
